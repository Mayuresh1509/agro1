...
model = pickle.load(open(r'C:\Users\ASUS\Downloads\Crop-Recommendation-main\Crop-Recommendation-main\classifier.pkl', 'rb'))
...
def predict(n, p, k, temp, humi, ph, rain, output=None):
    output
    ...
    for i in range(0, len(classes)):
        if classes.encoded[i] == pred:
            output: object = classes.index[i].upper()
    return output
...
def main():
    b1, titl, b2 = st.columns([1,10000,1])
    titl.title('Agro Advisor - A Crop Recommendation System')
